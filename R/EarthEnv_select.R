...
EarthEnv_select <- function(){
  earthenv_files_list <- list("hydro_avg", "soil_avg", "soil_min", "soil_max", "elevation", "slope")
  ...
  soil_avg_selected <- soil_avg[[c(2)]]
  soil_min_selected <- soil_min[[c(2)]]
  ...
  names(soil_avg_selected) <- paste(c("ph_avg"))
  names(soil_min_selected) <- paste(c("ph_min"))
  ...
  result <- stack(hydro_selected, soil_avg_selected, soil_min_selected, soil_max_selected, elevation_selected, slope_selected)
  return(result)
}
